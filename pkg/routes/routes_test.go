...
func TestRoute(t *testing.T) {
	...
	src := gcm.Coordinate{13.388860, 52.517037}
	dst := gcm.Coordinate{13.397634, 52.529407}
...
func TestRoutes(t *testing.T) {
	ctx := context.Background()
	src := gcm.Coordinate{13.388860, 52.517037}
	dstNear := gcm.Coordinate{13.397634, 52.529407}
	dstFar := gcm.Coordinate{14.397634, 53.529407}
	r, err := GetRoutes(ctx, src, []gcm.Coordinate{dstFar, dstNear})
	require.NoError(t, err)
